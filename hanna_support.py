    ...
    for message in messages:
        if message.author.name == "ハンナ":
            ...
            if condition.startswith("```md"):
                # "残り〜凸"と空行を引く
                left3 = condition[condition.find("# 残り3凸"):condition.find("# 残り2凸")].count("\n") - 2
                send_message += "3凸：" + str(left3) + "人\n"

                left2 = condition[condition.find("# 残り2凸"):condition.find("# 残り1凸")].count("\n") - 2
                send_message += "2凸：" + str(left2) + "人\n"

                left1 = condition[condition.find("# 残り1凸"):condition.find("# 残り0凸")].count("\n") - 2
                send_message += "1凸：" + str(left1) + "人\n"

                left_all = left3 + left2 + left1
                send_message += "あと" + str(left_hour) + "時間で" + str(left_all) + "凸\n"
